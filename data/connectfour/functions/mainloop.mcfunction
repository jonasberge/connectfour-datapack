...
execute as @e[tag=connectfour_state,sort=nearest,limit=1] run execute if entity @s[tag=_game_finished,tag=_mark_winning_blocks] run execute as @e[tag=connectfour_winning_block,sort=nearest,limit=1] at @s run execute unless entity @e[tag=connectfour_falling_block,distance=..10] run function connectfour:mark_winning_blocks
